...
fn main() {
    ...
    if let Some(command) = args.command {
        match command {
            Commands::Bench { size, iterations, use_512, chunked } => {
...
fn run_benchmark(size: usize, iterations: usize, use_512: bool, chunked: bool) {
    ...
    println!("Method: {}", if chunked { "chunked" } else { "single-pass" });
    println!();
    ...
    for _ in 0..10 {
        if use_512 {
            let _ = ChaosFiber256::hash512(&data);
        } else {
            let _ = ChaosFiber256::hash(&data);
        }
    }
    ...
    if chunked {
        ...
        for _ in 0..iterations {
            if use_512 {
                let hash512 = ChaosFiber256::hash512(&data);
                hash.copy_from_slice(&hash512[..32]);
            } else {
                hash = ChaosFiber256::hash(&data);
            }
        }
    }
    ...
    println!("Nanoseconds per byte: {:.2}", (elapsed.as_nanos() as f64) / total_bytes);
    if !use_512 {
        println!("Cycles/byte (est. at 3GHz): {:.2}", (elapsed.as_nanos() as f64 * 3.0) / total_bytes);
    }
}
...
fn hash_stdin(args: &Args) -> io::Result<()> {
    ...
    let hash = if args.use_512 {
        hex::encode(ChaosFiber256::hash512(&buffer))
    } else {
        hex::encode(ChaosFiber256::hash(&buffer))
    };
...
fn check_hashes(check_file: &PathBuf, args: &Args) -> io::Result<()> {
    ...
    for line in reader.lines() {
        ...
        if parts.len() != 2 {
            if args.warn {
                eprintln!("{}:{}: improperly formatted line", check_file.display(), line_num);
            }
            continue;
        }
...
mod tests {
    ...
    fn test_hash_consistency() {
        ...
        let hash1 = hex::encode(ChaosFiber256::hash(data));
        let hash2 = hex::encode(ChaosFiber256::hash(data));
        assert_eq!(hash1, hash2);

        let hash512_1 = hex::encode(ChaosFiber256::hash512(data));
        let hash512_2 = hex::encode(ChaosFiber256::hash512(data));
